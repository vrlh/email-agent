"""Google Gemini provider — free tier (1500 req/day on gemini-2.0-flash)."""
...
def _chat(system: str, user_prompt: str) -> str:
    client = _get_client()
...
def triage_emails(emails: List[Email]) -> List[TriageResult]:
    ...
    try:
        ...
        logger.error(f"Gemini triage failed: {exc}")
        return [
            TriageResult(
...
def summarize_email(email: Email) -> str:
    try:
        return _chat(SUMMARIZE_SYSTEM, build_summarize_user_prompt(email))
    except Exception as exc:
