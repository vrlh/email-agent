...
State format: either ``<SETUP_SECRET>`` (initial-add flow, no target account)
or ``<SETUP_SECRET>::<hint_email>`` (reauth flow — we verify the user signed
in as the requested account and reject the upsert on mismatch).
"""
...
def _parse_state(state: Optional[str], expected_secret: str) -> Tuple[bool, Optional[str]]:
    """Decode the OAuth ``state`` param into ``(secret_valid, hint)``.

    Accepts both legacy bare-secret format and the new ``<secret>::<hint>``
    format so links generated by older agent versions still work.
    """
    if not state or not expected_secret:
        return False, None
    if _STATE_SEP in state:
        secret_part, hint = state.split(_STATE_SEP, 1)
    else:
        secret_part, hint = state, None
    return (secret_part == expected_secret), hint


def _hint_matches(hint: Optional[str], returned_email: str) -> bool:
    """True iff the user signed in as the requested account.

    Returns True when ``hint`` is None (no target = nothing to verify, as in
    the initial-add flow). Comparison is case-insensitive because Gmail
    addresses are case-insensitive.
    """
    if hint is None:
        return True
    return hint.strip().lower() == (returned_email or "").strip().lower()
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        secret_valid, hint = _parse_state(state, os.environ.get("SETUP_SECRET", ""))
        if not secret_valid or not code:
        ...
        try:
            ...
            if not _hint_matches(hint, email_address):
                self._respond(
                    400,
                    _mismatch_page(hint, email_address),
...
def _mismatch_page(expected: Optional[str], got: str) -> str:
    """HTML page shown when the user signs in as the wrong Google account."""
...
<style>
  ...
  body {{ background: #f5f7fa; color: #1a1a1a; display: flex; align-items: center; justify-content: center; }}
  ...
  .x {{ width: 56px; height: 56px; border-radius: 50%; background: #ef4444;
...
<div class="card">
  ...
  <p>You signed in as <span class="email">{got}</span> but the reauth link was for <span class="email">{expected}</span>.</p>
  <p>Nothing was changed. Close this tab, go back to Slack, and click the reauth link again — make sure to pick <span class="email">{expected}</span> in Google's account chooser.</p>
  <p class="hint">If you wanted to add <span class="email">{got}</span> as a new account, ask the assistant for an "add account" link instead.</p>
</div>
</body>
</html>"""
